...
Now let's pull the sequence for the top hit contig. Below the code will check if the sequence identifier already exists in the extracted_sequenences.fasta file prior to reprinting it into the file. ]


```{r fasta file for extracted sequences, engine= 'bash', eval = FALSE}
...
# Output file
output_file="output/extracted_sequences.fasta"

# Loop through the identifiers
for identifier in "${!gene_map[@]}"; do
    gene_name="${gene_map[$identifier]}"
    # Check if the identifier exists in the output file
    if ! grep -q "$identifier $gene_name" "$output_file"; then
        # Extract the sequence and append it to the output file
        awk -v id="$identifier" -v gene="$gene_name" '$0 ~ id {flag=1; print id,gene; next} /^>/{flag=0} flag {printf "%s", $0}' data/botznik-ctg.fa >> "$output_file"
    fi
done
