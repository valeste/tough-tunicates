...
Before we use PrimerQuest, we want to identify the open reading frames of the GOI sequences **if** the sequence has not already been identified as the complete CDS on NCBI. An open reading frame (ORF) is a portion of a DNA molecule that, when translated into amino acids, contains no stop codons. Doing this step will improve the specificity of our primers as to make sure we are targeting the protein coding region of the gene.
